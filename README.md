...
`insurance_dataset.csv` — **10,290 rows x 13 columns**, with approximately **2% missing values**.
...
| `Salary` | Gross monthly salary (euros) |
| `Area` | Geographic living area |
| `Children` | Binary, has children (1 = Yes) |
| `CMV` | Customer Monetary Value |
| `Claims` | Claims rate |
| `Motor` | Motor insurance premium (euros) |
| `Household` | Household insurance premium (euros) |
| `Health` | Health insurance premium (euros) |
...
3. Feature Engineering
   |-- total_premium   = sum of all premium types
   |-- commitment      = annual_salary / total_premium
   |-- profit_percent  = ((total_premiums - claim_amount) / claim_amount) x 100
   |-- retention_cost  = (profit x no. of years) - CMV
...
7. Model Evaluation and Selection
   ...
   |
   v
8. Post-Clustering EDA and Segment Profiling
   |-- Median profit % per cluster
   |-- Median commitment and age per cluster
...
1. **Focus on middle-aged customers (Cluster 0 and 3)** for standard insurance plans — reliable payers with established household and motor needs.
2. **Target young adults (Clusters 1 and 5)** with basic, entry-level insurance plans to build long-term relationships early.
3. **Tailor premium health and life offerings for retired adults (Clusters 2 and 4)**, who have high purchasing power and low risk.
...
6. **Target middle-aged individuals for motor insurance**, and young adults earning around 2,000 euros/mo for household and work compensation insurance.
...
|-- data/
|   +-- insurance_dataset.csv    # Input dataset
